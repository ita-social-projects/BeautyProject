...
def get_free_time(specialist, order_date, working_day):
    ...
    free_time = [
        string_to_time(time).time() for time in working_day
    ]
...
def get_free_time_specialist_for_owner(specialist, order_date, working_day):
...
class OwnerSpecialistScheduleView(APIView):
    ...
    def get(self, request, position_id, specialist_id, order_date):
        ...
        if specialist not in position.specialist.all():
            return Response(
                {"detail": "Such specialist doesn't hold this position"},
                status=status.HTTP_404_NOT_FOUND,
            )

        return Response(
            get_free_time_specialist_for_owner(
                specialist, order_date, working_day,
            ),
            status=status.HTTP_200_OK,
        )
